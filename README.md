...
找到resources下的application.properties文件，修改DASH_API_KEY（这个需要注册阿里云账号，点击大模型导航栏，再点击通义千问max，然后获取API_KEY，将API_KEY配置到系统环境变量中，环境变量名就叫DASH_SCOPE_API_KEY）
...
配置Pinecone的环境变量，官网注册并登录（科学上网）,获取API_KEY并配置为环境变量，变量名为PINECONE_API_KEY
安装Node.js（node-v18.17.1-x64.msi）
配置npm镜像，打开cmd 执行npm config set registry https://registry.npmmirror.com
...
参考资料
本项目是尚硅谷的小智医疗项目的源码，若需要更详细的了解，请移步B站看环环老师的讲解视频。
